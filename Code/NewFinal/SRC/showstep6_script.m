...
set(handles.finalframe_slider,'Min',handles.initialframe_num,'Max',numel(plant_obj.img_array),'Value',handles.initialframe_num,'SliderStep',[1/numel(plant_obj.img_array) 10/numel(plant_obj.img_array)]);

% set default value for final frame
handles.finalframe_num = handles.initialframe_num;

% show the default final frame
handles.finalframe = crop_func(handles.finalframe_num, plant_obj.img_array, handles.img_cropwidth ,handles.img_cropheight, handles.stalk_x, handles.stalk_y); 
axes(handles.axes1);
