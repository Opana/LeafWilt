...
% user selects location of analysis results
result_directory = uigetdir('','Select Destination Folder');

% if the img_directory isn't empty, an array of objects is created
if img_directory ~= 0
    handles.plant_obj_array = img_finder(img_directory, result_directory);
end

% hide welcome screen objects
hidewelcome_script;

% initializes plant_num
handles.plant_num = 1;
...
% select initial frame picture
plant_obj = handles.plant_obj_array(handles.plant_num);
handles.initialframe = plant_obj.img_array{handles.initialframe_num};
...
% run crop function
plant_obj = handles.plant_obj_array(handles.plant_num);
handles.initialframe = crop_func(handles.initialframe_num, plant_obj.img_array, handles.img_cropwidth, handles.img_cropheight, handles.stalk_x, handles.stalk_y);
...
plant_obj = handles.plant_obj_array(handles.plant_num);
plant_obj.sharpenradius = handles.sharpenradius;
...
plant_obj = handles.plant_obj_array(handles.plant_num);
handles.finalframe = crop_func(handles.finalframe_num, plant_obj.img_array, handles.img_cropwidth ,handles.img_cropheight, handles.stalk_x, handles.stalk_y);  % crop frame
axes(handles.axes1);
...
% set the object values that are necessary for analysis
plant_obj = handles.plant_obj_array(handles.plant_num);
set(plant_obj, 'rect_array', handles.rect_array);
set(plant_obj, 'initialframe_num', handles.initialframe_num);
set(plant_obj, 'finalframe_num', handles.finalframe_num);
set(plant_obj, 'initialthreshold', handles.initialthreshold);
set(plant_obj, 'finalthreshold', handles.finalthreshold);
set(plant_obj, 'sharpenradius', handles.sharpenradius);
set(plant_obj, 'rgb_channel', handles.rgb_channel);
set(plant_obj, 'invert_TF', handles.invert_TF);
set(plant_obj, 'stalk_x', handles.stalk_x);
set(plant_obj, 'stalk_y', handles.stalk_y);
set(plant_obj, 'img_cropwidth', handles.img_cropwidth);
set(plant_obj, 'img_cropheight', handles.img_cropheight);

% loops the program until every timelapse folder has been set up for
% analysis
[~, length] = size(handles.plant_obj_array);
if handles.plant_num < length
    handles.plant_num = handles.plant_num+1;
    showstep2_script;
    guidata(handles.output,handles);
    
% performs the wiltanalysis for each object
else
    for n = 1:length
        handles.plant_num = n;
        plant_obj = handles.plant_obj_array(n);
        angle_data = wiltanalysis_func(plant_obj);
        set(plant_obj, 'angle_data', angle_data);
        
        % produces a scatterplot of the data and provides the leaf numbers
        showstep9_script;
        
        % creates an absolute filename and produces a csv file of the data
        file = strcat(plant_obj.dest_directory, plant_obj.filename, 'Data', '.csv');
        csvwrite(file, plant_obj.angle_data');
    end
end
