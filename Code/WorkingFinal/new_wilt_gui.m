...
handles.result_directory = uigetdir('','Select Destination Folder');

% img_array_func saves all images in user selected directory into a cell
% array. Will also look at all image sets in a folder and store those for
% further analysis.
if img_directory ~= 0
    handles.img_array = img_array_func(img_directory);
    handles.img_matrix = [handles.img_array];
    handles.filename_array = string(getfilename(img_directory));
    ...
    if ~isempty(subFolders)
        handles.result_directory
        handles.filename_array = {};
        for k = 1 : length(subFolders)
            handles.img_array = img_array_func(strcat(img_directory, '\', subFolders(k).name));
            handles.img_matrix = [handles.img_matrix; handles.img_array];
            handles.filename_array{end+1} = strcat('\', subFolders(k).name);
        end
    end
    
    handles.img_array = handles.img_matrix(1,:);
end

% hide welcome screen objects
hidewelcome_script;

% initalize array of wilt data
handles.wiltdata_array = [];
handles.initialframe_array = [];
handles.finalframe_array = [];
...
handles.initialframe_num = get(handles.initialframe_slider, 'value');
handles.initialframe_array = [handles.initialframe_array handles.initialframe_num];
handles.finalframe_num = get(handles.finalframe_slider, 'value');
handles.finalframe_array = [handles.finalframe_array handles.finalframe_num];

% creates a wiltdata object that is then stored in an array of wiltdata
% objects
wiltdata = wiltdata_creator(handles.img_array, handles.rect_array, handles.initialframe_num, handles.finalframe_num, handles.initialthreshold, handles.finalthreshold, handles.sharpenradius, handles.rgb_channel, handles.invert_TF, handles.stalk_x, handles.stalk_y, handles.img_cropwidth, handles.img_cropheight);
handles.wiltdata_array = [handles.wiltdata_array, wiltdata];
handles.img_matrix(1,:) = [];

% creates a loop to analyze every dataset in the matrix until all datasets
% have been analyzed
if ~isempty(handles.img_matrix)
    handles.img_array = handles.img_matrix(1,:);
    showstep2_script;
    guidata(handles.output,handles);
else
% analyzes each wiltdata object and creates a separate csv file for each of
% them
    for n = 1:length(handles.wiltdata_array)
        handles.wilt_data = wiltanalysis_func(handles.wiltdata_array(n));
        filename = string(handles.filename_array(n));
        showstep9_script;
        handles.initialframe_array(1) = [];
        handles.finalframe_array(1) = [];
        file = strcat(handles.result_directory, filename, 'Data', '.csv');
        csvwrite(file, handles.wilt_data');
    end
end
...
% creates filename for csv output
...
% creates wildata object
function [wiltdata] = wiltdata_creator(img_array, rect_array, initialframe_num, finalframe_num, initialthreshold, finalthreshold, sharpenradius, rgb_channel, invert_TF, stalk_x, stalk_y, img_cropwidth, img_cropheight)
wiltdata = wiltdata_constructor;
wiltdata.img_array = img_array;
wiltdata.rect_array = rect_array;
wiltdata.initialframe_num = initialframe_num;
wiltdata.finalframe_num = finalframe_num;
wiltdata.initialthreshold = initialthreshold;
wiltdata.finalthreshold = finalthreshold;
wiltdata.sharpenradius = sharpenradius;
wiltdata.rgb_channel = rgb_channel;
wiltdata.invert_TF = invert_TF;
wiltdata.stalk_x = stalk_x;
wiltdata.stalk_y = stalk_y;
wiltdata.img_cropwidth = img_cropwidth;
wiltdata.img_cropheight = img_cropheight;